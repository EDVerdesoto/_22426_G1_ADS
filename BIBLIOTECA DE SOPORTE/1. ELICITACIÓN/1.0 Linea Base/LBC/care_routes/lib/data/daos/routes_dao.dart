...
class RoutesDao extends DatabaseAccessor<AppDatabase> with _$RoutesDaoMixin {
  ...
  Future<int> insertRouteAndStops(
    Route ,
  ) async {
    final routeId = await into(routes).insert(route.toCompanion());
    for (final stop in stops) {
      final stopWithRouteId = stop.copyWith(routeID: Value(routeId));
      await into(routeStops).insert(stopWithRouteId.toCompanion());
    }
    return routeId;
  }
